...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($id_jadwal)) {
        ...
        if (mysqli_query($mysqli, $insert_query)) {
            // Ambil jam mulai dan jam selesai dari jadwal yang dipilih
            $query_jadwal = "SELECT jam_mulai, jam_selesai FROM jadwal WHERE id = '$id_jadwal'";
            $result_jadwal = mysqli_query($mysqli, $query_jadwal);
            if ($row_jadwal = mysqli_fetch_assoc($result_jadwal)) {
            ...
            header("Location: " . $_SERVER['PHP_SELF'] . "?status=success&no_antrian=$no_antrian&jam_mulai=$jam_mulai&jam_selesai=$jam_selesai");
